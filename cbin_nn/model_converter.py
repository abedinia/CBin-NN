...
# Patch for Keras 3 / TF compatibility where LocallyConnected1D is missing but required by Larq
if not hasattr(tf.keras.layers, 'LocallyConnected1D'):
    class LocallyConnected1D(tf.keras.layers.Layer):
        pass
    tf.keras.layers.LocallyConnected1D = LocallyConnected1D
...
sys.modules['larq.optimizers'] = MagicMock()

import larq as lq
import numpy as np
import math
import torch
from . import save_params as sp

import argparse

def main():
    # User defined params:
    parser = argparse.ArgumentParser(description='Convert Keras model to C headers for CBin-NN.')
    parser.add_argument('model', type=str, help='Path to the input .h5 model file')
    parser.add_argument('output_dir', type=str, nargs='?', default='models', help='Output directory for generated files')
    ...
    sp.initialize_files(output_dir)
    ...
    # Load model with custom objects for Larq layers
    custom_objects = {'QuantConv2D': lq.layers.QuantConv2D, 'QuantDense': lq.layers.QuantDense}
    model = tf.keras.models.load_model(model_name, compile = False, custom_objects=custom_objects)
    ...
    with lq.context.quantized_scope(True):
        for layer_idx, layer in enumerate(model.layers):
            ...
            if 'conv' in layer.name:
                ...
                sp.save_bnn_params(weights, bias, layer_name, Conv_idx, in_shape, out_shape, ker_size, stride, padding, first_file_write, in_buffer, out_buffer, BN_idx, output_dir)
                Conv_idx += 1
                ...
                sp.save_bnn_params(weights, bias, layer_name, FC_idx, in_dim, out_dim, NULL, NULL, NULL, first_file_write, in_buffer, out_buffer, BN_idx, output_dir)
                FC_idx += 1
                first_file_write = False
                if in_buffer == 'buffer1':
                    in_buffer = 'buffer2'
                ...
                if BN_idx == 1:
                    ...
                    sp.save_bnn_params(alpha, NULL, layer_name, BN_idx, dim, NULL, NULL, NULL, NULL, first_file_write, NULL, NULL, NULL, output_dir)
                    BN_idx = BN_idx + 1
                # All other BN layers are handled as follows, for more information refer to the paper
                    ...
                    sp.save_bnn_params(integer_bias, NULL, layer_name, BN_idx, dim, NULL, NULL, NULL, NULL, first_file_write, NULL, NULL, NULL, output_dir)
                    BN_idx = BN_idx + 1
                first_file_write = False
                ...
                sp.save_bnn_params(PReLU_weights, NULL, layer_name, PReLU_idx, dim, NULL, NULL, NULL, NULL, first_file_write, NULL, NULL, NULL, output_dir)
                ...
                sp.save_bnn_params(NULL, NULL, layer_name, MaxPool_idx, in_shape, out_shape, pool_size, stride, padding, first_file_write, in_buffer, out_buffer, NULL, output_dir)
            ...
            ### To be tested 
                ...
                sp.save_bnn_params(NULL, NULL, layer_name, NULL, NULL, n_classes, NULL, NULL, NULL, first_file_write, NULL, NULL, NULL, output_dir)
                first_file_write = False
    ...
    sp.save_bnn_params(NULL, NULL, layer_name, NULL, max_activation, second_max_activation, NULL, NULL, NULL, first_file_write, NULL, NULL, NULL, output_dir)
